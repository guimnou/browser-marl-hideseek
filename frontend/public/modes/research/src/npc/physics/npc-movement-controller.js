...
export class NPCMovementController {
  constructor(scene, chunkManager) {
    ...
    if (!chunkManager) {
      console.warn(
        "⚠️ NPCMovementController: No chunkManager provided - block operations disabled"
      );
    }
    ...
    // Set chunkManager on block systems
    ...
    // ⭐ REMOVED: Don't cache block limits (breaks curriculum learning)
  ...
  executeActionGroups(npc, actions, deltaTime) {
    ...
    const speed =
      npc.role === "seeker"
        ? this.physics.SPRINT_SPEED // Seeker runs faster
        : this.physics.WALK_SPEED; // Hider walks normally
  ...
  placeBlock(npc) {
    // ⭐ FIXED: Read from config directly (supports curriculum)
    const maxBlocksPlaced = NPC.BLOCK_PLACEMENT?.maxBlocksPlaced || 0;

    if (npc.blocksPlaced >= maxBlocksPlaced) {
      return {
        success: false,
        action: "place_block",
    ...
    // Check if blocks are enabled (maxBlocksPlaced > 0)
    if (maxBlocksPlaced === 0) {
      return {
        success: false,
        action: "place_block",
        reason: "blocks_disabled",
      };
    }

    // Check if block system is available
  ...
  removeBlock(npc) {
    // ⭐ FIXED: Read from config directly (supports curriculum)
    const maxBlocksRemoved = NPC.BLOCK_REMOVAL?.maxBlocksRemoved || 0;

    if (npc.blocksRemoved >= maxBlocksRemoved) {
      return {
        success: false,
        action: "remove_block",
        reason: "limit_reached",
    ...
    // Check if blocks are enabled (maxBlocksRemoved > 0)
    if (maxBlocksRemoved === 0) {
      return {
        success: false,
        action: "remove_block",
        reason: "blocks_disabled",
