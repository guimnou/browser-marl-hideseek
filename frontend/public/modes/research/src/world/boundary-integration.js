...
export function isNearResearchBoundary(position, threshold = 2) {
  ...
  const boundaryHeight = researchBoundarySystem.BOUNDARY_HEIGHT;
  ...
  return (
    ...
    position.y > boundaryHeight - threshold
  );
}
...
export function getSafeNPCSpawnPosition(worldConfig) {
  ...
  return new THREE.Vector3(
    ...
    worldConfig.BASE_GROUND_LEVEL + 10,
    buffer + Math.random() * (worldSize - buffer * 2)
  );
}
...
export function quickSetupBoundaries(scene, worldConfig) {
