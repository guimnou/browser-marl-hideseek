...
class TreeGenerator {
  ...
  generateTreesInChunk(chunk, chunkX, chunkY, chunkZ, heightMap, biomeMap) {
    ...
    for (let x = 0; x < this.clientConfig.CHUNK_SIZE; x++) {
      for (let z = 0; z < this.clientConfig.CHUNK_SIZE; z++) {
        ...
        if (
          Math.abs(worldX) <= this.worldConfig.SIZE &&
          Math.abs(worldZ) <= this.worldConfig.SIZE
        ) {
  ...
  getPositionSeed(worldX, worldZ) {
    ...
    return (
      ((worldX * 73856093) ^ (worldZ * 19349663) ^ this.worldConfig.SEED) /
      Math.pow(2, 32)
    );
  }
  ...
  hasEnoughSpacing(x, z, treePositions) {
    for (
      let dx = -this.worldConfig.TREES.INFLUENCE_RADIUS;
      dx <= this.worldConfig.TREES.INFLUENCE_RADIUS;
      dx++
    ) {
      for (
        let dz = -this.worldConfig.TREES.INFLUENCE_RADIUS;
        dz <= this.worldConfig.TREES.INFLUENCE_RADIUS;
        dz++
      ) {
        const checkX = x + dx;
        const checkZ = z + dz;
        ...
        if (
          checkX < 0 ||
          checkX >= this.clientConfig.CHUNK_SIZE ||
          checkZ < 0 ||
          checkZ >= this.clientConfig.CHUNK_SIZE
        ) {
          continue;
        }
  ...
  shouldPlaceTree(worldX, worldZ, biomeType, surfaceHeight) {
    if (
      surfaceHeight <=
      this.worldConfig.WATER_LEVEL + this.worldConfig.TREES.BEACH_BUFFER
    ) {
      return false;
    }
  ...
  isValidTreePosition(
    ...
    const requiredFlatRadius =
      this.worldConfig.TREES.PLACEMENT.REQUIRED_FLAT_RADIUS;
    const maxHeightDiff =
      this.worldConfig.TREES.PLACEMENT.MAX_HEIGHT_DIFFERENCE;
    ...
    for (let dx = -requiredFlatRadius; dx <= requiredFlatRadius; dx++) {
      for (let dz = -requiredFlatRadius; dz <= requiredFlatRadius; dz++) {
        ...
        const neighborHeight =
          heightMap[checkX + checkZ * this.clientConfig.CHUNK_SIZE];
  ...
  generateTree(chunk, x, y, z, biomeType) {
    ...
    if (
      x < leafConfig.MAX_RADIUS ||
      x >= this.clientConfig.CHUNK_SIZE - leafConfig.MAX_RADIUS ||
      z < leafConfig.MAX_RADIUS ||
      z >= this.clientConfig.CHUNK_SIZE - leafConfig.MAX_RADIUS ||
      y < 1 ||
      y >=
        this.clientConfig.CHUNK_SIZE -
          (biomeConfig.TRUNK_HEIGHT.MAX + leafConfig.LAYERS.length)
    ) {
      return;
    }
    ...
    const heightRange =
      biomeConfig.TRUNK_HEIGHT.MAX - biomeConfig.TRUNK_HEIGHT.MIN;
    const height =
      biomeConfig.TRUNK_HEIGHT.MIN + Math.floor(randValue * heightRange);
  ...
  getBlockIndex(x, y, z) {
    if (
      x < 0 ||
      x >= this.clientConfig.CHUNK_SIZE ||
      y < 0 ||
      y >= this.clientConfig.CHUNK_SIZE ||
      z < 0 ||
      z >= this.clientConfig.CHUNK_SIZE
    ) {
      return -1;
    }
    return (
      x +
      y * this.clientConfig.CHUNK_SIZE +
      z * this.clientConfig.CHUNK_SIZE * this.clientConfig.CHUNK_SIZE
    );
  }
...
class CaveGenerator {
  ...
  generateCaves(x, y, z) {
    if (
      y < this.worldConfig.CAVES.MIN_HEIGHT ||
      y > this.worldConfig.CAVES.MAX_HEIGHT
    ) {
      return false;
    }
    ...
    const isSpaghettiCave =
      spaghettiNoise > this.worldConfig.CAVES.SPAGHETTI.THRESHOLD;
  ...
  generateTunnel(x, y, z) {
    ...
    const heightBias = Math.exp(
      (-(y - config.PREFERRED_HEIGHT) * (y - config.PREFERRED_HEIGHT)) /
        (2 * config.HEIGHT_VARIATION * config.HEIGHT_VARIATION)
    );

    // Connect nearby tunnels (creates intersections)
    const connectionNoise =
      this.simplexNoise.noise3D(
  ...
  generateSpaghetti(x, y, z) {
    ...
    return (
      mainNoise +
      windingNoise +
      (branchNoise > config.BRANCH_THRESHOLD ? branchNoise : 0)
    );
  }
}
...
class OreGenerator {
  ...
  generateOre(x, y, z) {
    if (
      y >= worldConfig.ORES.COAL.MIN_HEIGHT &&
      y <= worldConfig.ORES.COAL.MAX_HEIGHT
    ) {
      const noiseCoal = simplexNoise.noise3D(
        x * worldConfig.ORES.COAL.SCALE,
        y * worldConfig.ORES.COAL.SCALE,
        z * worldConfig.ORES.COAL.SCALE
      );
      if (noiseCoal > worldConfig.ORES.COAL.THRESHOLD)
        return block_type.COAL_ORE;
    }

    if (
      y >= worldConfig.ORES.IRON.MIN_HEIGHT &&
      y <= worldConfig.ORES.IRON.MAX_HEIGHT
    ) {
      const noiseIron = simplexNoise.noise3D(
        x * worldConfig.ORES.IRON.SCALE,
        y * worldConfig.ORES.IRON.SCALE,
        z * worldConfig.ORES.IRON.SCALE
      );
      if (noiseIron > worldConfig.ORES.IRON.THRESHOLD)
...
importScripts("./noise/simplex-noise.js", "./schematic/schematic_handler.js");
...
self.onmessage = async function (e) {
  switch (e.data.type) {
    case "init":
      try {
        ...
        const response = await fetch(
          "/src/web-worker/trees/patterns/tree.schem"
        );
        const buffer = await response.arrayBuffer();

        const loadSuccess = await schematicHandler.loadSchematic(
          buffer,
          "tree"
        );
    ...
    case "applyModifications":
      ...
      console.log(
        `Worker received ${modifications.length} modifications to apply`
      );

      modifications.forEach((mod) => {
        const key = createModificationKey(
          mod.chunkX,
          mod.chunkY,
          mod.chunkZ,
          mod.localX,
    ...
    case "updateBlock":
      const { chunkX, chunkY, chunkZ, localX, localY, localZ, blockType } =
        e.data;
      const modKey = createModificationKey(
        chunkX,
        chunkY,
        chunkZ,
        localX,
    ...
    case "loadSchematic":
      if (schematicHandler) {
        try {
          const { buffer, patternName, metadata } = e.data;
          // Pass metadata directly to loadSchematic
          const success = await schematicHandler.loadSchematic(
            buffer,
            patternName,
            metadata
          );
...
function createModificationKey(chunkX, chunkY, chunkZ, localX, localY, localZ) {
  ...
  if (
    localX < 0 ||
    localX >= client_config.CHUNK_SIZE ||
    localY < 0 ||
    localY >= client_config.CHUNK_SIZE ||
    localZ < 0 ||
    localZ >= client_config.CHUNK_SIZE
  ) {
    return null;
  }
...
function generateChunk(chunkX, chunkY, chunkZ) {
  ...
  const heightMap = new Array(
    client_config.CHUNK_SIZE * client_config.CHUNK_SIZE
  );
  const biomeMap = new Array(
    client_config.CHUNK_SIZE * client_config.CHUNK_SIZE
  );

  const worldYStart = chunkY * client_config.CHUNK_SIZE;
  const worldYEnd = worldYStart + client_config.CHUNK_SIZE - 1;
  ...
  for (let x = 0; x < client_config.CHUNK_SIZE; x++) {
    for (let z = 0; z < client_config.CHUNK_SIZE; z++) {
      ...
      if (
        Math.abs(worldX) <= worldConfig.SIZE &&
        Math.abs(worldZ) <= worldConfig.SIZE
      ) {
    ...
    for (let z = 0; z < client_config.CHUNK_SIZE; z++) {
      ...
      if (
        ...
        for (let y = 0; y < client_config.CHUNK_SIZE; y++) {
          ...
          const index =
            x +
            y * client_config.CHUNK_SIZE +
            z * client_config.CHUNK_SIZE * client_config.CHUNK_SIZE;
  ...
  // 🔍 DIAGNOSTIC: Log chunk info if it's near spawn area
  if (chunkX === 0 && chunkZ === 0 && worldYStart <= 310 && worldYEnd >= 300) {
    console.log(
      `🔍 CHUNK (${chunkX}, ${chunkY}, ${chunkZ}): Y=${worldYStart}-${worldYEnd}, Blocks=${blockCount}, HasBlocks=${hasBlocks}`
    );

    // Check specific position (13, 304, 12) from your log
    const localX = 13 % client_config.CHUNK_SIZE;
    const localZ = 12 % client_config.CHUNK_SIZE;
    const heightAtSpawn = heightMap[localX + localZ * client_config.CHUNK_SIZE];
    console.log(`   At spawn pos (13, 12): Height=${heightAtSpawn}`);
  }

  // Generate trees
  if (treeGenerator) {
    treeGenerator.generateTreesInChunk(
      chunk,
      chunkX,
      chunkY,
      chunkZ,
...
function generateTerrainHeight(x, z) {
  ...
  for (let i = 0; i < octaves; i++) {
    noise +=
      simplexNoise.noise2D(x * scale * frequency, z * scale * frequency) *
      amplitude;
