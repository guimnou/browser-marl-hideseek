...
export class ChunkManager {
  ...
  async processQueues() {
    ...
    try {
      ...
      while (
        ...
        processedCount < this.maxChunksPerFrame * 5
      ) {
        // Allow more block updates per frame
  ...
  generateChunk(chunkX, chunkZ) {
    ...
    if (!chunkBuffer) {
      console.error(
        `Failed to generate chunk at (${chunkX}, ${chunkZ}) due to buffer allocation failure.`
      );
      return;
    }
  ...
  handleChunkData(chunk, chunkX, chunkY, chunkZ) {
    if (!this.createMeshFunction || !this.isChunkInBounds(chunkX, chunkZ))
      return;
  ...
  processPendingNetworkUpdates() {
    if (
      !GameState.isOnline ||
      !GameState.socket ||
      this.pendingNetworkUpdates.size === 0
    ) {
      return;
    }
...
class BufferPool {
  ...
  getBuffer() {
    ...
    if (bufferSize > this.maxBufferSize) {
      console.error(
        `Requested buffer size ${bufferSize} exceeds maximum allowed size ${this.maxBufferSize}`
      );
      return null;
    }
