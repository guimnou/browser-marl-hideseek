...
const { PLAYER_HEIGHT, WALK_SPEED, SPRINT_SPEED, SNEAK_SPEED, FLY_SPEED } =
...
export function createPlayer(scene, playerData, textureAtlas, isLocalPlayer) {
  ...
  if (playerData.isNPC) {
    ...
    // Player uses GameState.spawn
    const spawnPosition = GameState.spawn(
      playerData.position?.x,
      playerData.position?.z
    );
    finalPosition = {
      x: playerData.position?.x || spawnPosition.x,
      y: playerData.position?.y || spawnPosition.y,
      z: playerData.position?.z || spawnPosition.z,
    };
  }

  // Set initial position
  player.position.set(finalPosition.x, finalPosition.y, finalPosition.z);
  ...
  player.yaw = playerData.rotation;
  player.pitch = 0;
